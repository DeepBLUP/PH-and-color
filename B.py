...
# 调整路径为您的文件实际路径
filepath = r'C:\python\2024\3\3屠宰\4.3\眼肌pH\pH有固定效应.csv'
# 尝试使用utf-8编码读取
...
# 保存结果为 CSV 文件
#results_df.to_csv(r'C:\python\2024\3\3屠宰\4.3\眼肌pH\眼肌pHB组.csv', index=False)
...
for name, model in models.items():
    ...
    plt.title(f'True Value vs Predicted Value: {name} B')

    # Save the figure
    file_name = f'{name.replace(" ", "_")}B.png'
    save_path = f'C:\\python\\2024\\3\\3屠宰\\4.3\\眼肌pH\\B组\\{file_name}'
